...
function ArchiveListView({ writings, onSwitchView }: { writings: Writing[]; onSwitchView: () => void }) {
  ...
  const grouped = useMemo(() => {
    const sorted = [...writings].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
  ...
  return (
    <div className="min-h-screen bg-[#020204] text-zinc-100">
      <div className="fixed inset-0 pointer-events-none bg-gradient-to-b from-indigo-950/5 via-transparent to-cyan-950/5" />

      <header className="fixed top-0 left-0 right-0 z-50 px-8 py-6 bg-gradient-to-b from-[#020204] to-transparent">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <Link href="/" className="text-[10px] tracking-[0.3em] uppercase text-zinc-500 hover:text-zinc-300 transition-colors">
            ← back
          </Link>
          <button onClick={onSwitchView} className="text-[9px] tracking-[0.2em] uppercase text-zinc-500 hover:text-zinc-300 px-4 py-2 border border-zinc-800 hover:border-zinc-600 rounded-lg transition-all">
            map view
          </button>
        </div>
      </header>
      ...
      <main className="pt-32 pb-20 px-8 relative z-10">
        <div className="max-w-4xl mx-auto">
          <h1 className={`text-4xl font-extralight tracking-wide mb-16 transition-all duration-700 ${isLoaded ? 'opacity-100' : 'opacity-0'}`} style={{ fontFamily: "var(--font-cormorant), serif" }}>
            Archive
          </h1>
            ...
            {years.map((year, yi) => (
              <section key={year} className={`transition-all duration-700 ${isLoaded ? 'opacity-100' : 'opacity-0'}`} style={{ transitionDelay: `${yi * 100}ms` }}>
                <div className="flex items-center gap-8 mb-6">
                  <h2 className="text-3xl font-extralight text-zinc-600" style={{ fontFamily: "var(--font-cormorant), serif" }}>{year}</h2>
                  ...
                  {grouped[year].map((w) => {
                    ...
                    return (
                      <Link
                        key={w.id}
                        href={`/reading/${w.id}`}
                        className="group flex items-center gap-6 py-4 px-4 -mx-4 rounded-lg transition-all duration-300"
                        style={{ background: isHovered ? `${color}08` : 'transparent' }}
                      ...
                      >
                        ...
                        <div className="flex-1 flex items-center justify-between">
                          <span className="text-base font-light transition-colors duration-300" style={{ color: isHovered ? color : '#a1a1aa', fontFamily: "var(--font-cormorant), serif" }}>
                            {w.title}
                          </span>
                          <div className={`flex items-center gap-3 transition-opacity duration-300 ${isHovered ? 'opacity-100' : 'opacity-0'}`}>
                            {w.tags.slice(0, 2).map(tag => (
                              <span key={tag} className="text-[9px] px-2 py-0.5 rounded-full" style={{ background: `${color}15`, color }}>
                                {tag}
                              </span>
                            ))}
                          </div>
                        </div>
                        <span className={`text-[10px] font-mono transition-colors duration-300 ${isHovered ? 'text-zinc-500' : 'text-zinc-600'}`}>
